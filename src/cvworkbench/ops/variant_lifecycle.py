...
_REGISTRY_MUTEXES: dict[str, Lock] = {}
...
def _registry_write_lock(config_path: Path):
    ...
    with _REGISTRY_MUTEXES_GUARD:
        mutex = _REGISTRY_MUTEXES.setdefault(path_key, Lock())
...
def _lock_registry_handle(handle: Any) -> None:
    try:
        import fcntl
    except ImportError:
        import msvcrt
...
def _unlock_registry_handle(handle: Any) -> None:
    try:
        import fcntl
    except ImportError:
        import msvcrt
