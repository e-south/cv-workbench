...
- `pdf.py` owns sanitization and layout correspondence; `policy.py` owns disclosure policy loading; `artifact.py` owns manifest eligibility.
...
- CLI adapters live in `cli/publication.py`; workflow descriptions live in `workspace/publication.py`. Neither may weaken the operations checks.
